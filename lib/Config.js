...
class Config {
   ...
   * If the object was not initially loaded from a path, write to the default config path.
   *
   * @param {String} [path] -- path to JSON file
   * @throws {Error} on write failure
   */
  write(path) {
    path = path || this.__path || this.defaultConfigPath;
  ...
  /**
   * Return the default config directory name.
  ...
  get defaultConfigPath() {
    return Path.join(this.defaultConfigDirectory, this.defaultConfigFilename);
  }
